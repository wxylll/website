...
<div class="container-fluid">
    <div class="title"><span>视频资源管理</span></div>
    <div class="articles">
...
<script>
    var news = [<c:forEach items="${videos}" var="video">
        {'id':'${video.id}', 'title':'${video.title}', 'releaseTime':'<fmt:formatDate value="${video.releaseTime}" pattern="yyyy-MM-dd HH:mm:ss"/>'},
        </c:forEach> ]
        ...
        table.on('tool(test)', function(obj){
            ...
            if(obj.event === 'del'){
                layer.confirm('确定要删除此视频吗？',
                    ...
                    function(index){
                        $.ajax({
                            url:'${pageContext.request.contextPath}/adminVideoDelete.action?id=' + data.id,
                            type:'get',
    ...
    function search() {
        ...
        $.ajax({
            url:'${pageContext.request.contextPath}/adminVideoSearch.action?title=' + text,
            type:'get',
            success:function (data) {
    ...
    function videoAdd() {
        var index = layer.open({
            ...
            fixed: false, //不固定
            maxmin: true,
            content: ['${pageContext.request.contextPath}/view/backstage/items/_video_add.jsp']
        })
    }
