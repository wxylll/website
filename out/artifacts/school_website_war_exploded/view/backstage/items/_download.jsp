...
<div class="container-fluid">
    <div class="title"><span>下载专区管理</span></div>
    <div class="articles">
        <table id="article_table" class="layui-hide" lay-filter="test">
            <button class="layui-btn layui-btn-normal" style="float: right" onclick="videoAdd()"><i class="layui-icon">&#xe654;</i>添加</button>
...
<script>
    var news = [<c:forEach items="${downloads}" var="download">
        {'id':'${download.id}', 'title':'${download.title}', 'releaseTime':'<fmt:formatDate value="${download.releaseTime}" pattern="yyyy-MM-dd HH:mm:ss"/>'},
        </c:forEach> ]
        ...
        table.on('tool(test)', function(obj){
            ...
            if(obj.event === 'del'){
                layer.confirm('确定要删除此文件吗？',
                    ...
                    function(index){
                        $.ajax({
                            url:'${pageContext.request.contextPath}/adminDownloadDelete.action?id=' + data.id,
                            type:'get',
    ...
    function search() {
        ...
        $.ajax({
            url:'${pageContext.request.contextPath}/adminDownloadSearch.action?title=' + text,
            type:'get',
            success:function (data) {
    ...
    function videoAdd() {
        var index = layer.open({
            ...
            fixed: false, //不固定
            maxmin: true,
            content: ['${pageContext.request.contextPath}/view/backstage/items/_download_add.jsp']
        })
    }
