...
<div class="container-fluid">
    <div class="title"><span>学习资料管理</span></div>
    <div class="articles">
...
<script>
    var news = [<c:forEach items="${studies}" var="study">
        {'id':'${study.id}', 'title':'${study.title}', 'releaseTime':'<fmt:formatDate value="${study.releaseTime}" pattern="yyyy-MM-dd HH:mm:ss"/>'},
        </c:forEach> ]
    ...
    function search() {
        ...
        $.ajax({
            url:'${pageContext.request.contextPath}/adminArticleSearch.action?cid=3&title=' + text,
            type:'get',
            success:function (data) {
    ...
    function documentAdd() {
        var index = layer.open({
            ...
            fixed: false, //不固定
            maxmin: true,
            content: ['${pageContext.request.contextPath}/view/backstage/items/_article_add.jsp?cid=3']
        })
        layui.layer.full(index);
    }
