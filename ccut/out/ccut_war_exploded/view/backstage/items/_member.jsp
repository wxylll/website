...
<div class="container-fluid">
    <div class="title"><span>成员管理</span></div>
    <div class="members">
...
<script>
    var data = [<c:forEach items="${members}" var="member">
            {'id':'${member.id}','name':'${member.name}','photo':'${member.photo}','position':'${member.position}','politicsStatus':'${member.politicsStatus}'},
        </c:forEach>]
    ...
    layui.use('table', function(){
        ...
        table.render({
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'position',title:'职务',unresize:true}
                ,{field: 'politicsStatus',title:'政治面貌',unresize:true}
                ,{title:'操作',unresize:true, templet:function(d) {
                        return "<button class='layui-btn btn_position' onclick='staffAlter("+ d.id + ")'>编辑</button>" +
    ...
    function memberAdd() {
        layer.open({
            ...
            fixed: false, //不固定
            maxmin: true,
            content: ['${pageContext.request.contextPath}/view/backstage/items/_member_add.jsp','no']
        })
    }

    function staffAlter(id) {
        layer.open({
            title:'成员信息',
            type: 2,
            offset:100,
            ...
            fixed: false, //不固定
            maxmin: true,
            content: ['${pageContext.request.contextPath}/getMember.action?id=' + id,'no']
        })
    }
    ...
    function deleteStaff(id) {
        layer.confirm('确定要删除此成员？',
            ...
            function(index){
                $.ajax({
                    url:'${pageContext.request.contextPath}/adminMemberDelete.action?id=' + id,
                    type:'get',
                    success:function () {
                        layer.msg('删除成功！')
